...
def call_deepseek_write_chapter(
    ...
    system_prompt = (
        ...
        "4. **文献引用规范**：在文中陈述他人观点或使用数据时，必须极其自然地插入引用标记。你**只能**使用「可用参考文献列表」中的文献。引用时，**必须**输出占位符标记：[REF_{文献ID}]，例如 [REF_ref_001]。**确保占位符不要被反引号包围！** 系统随后会自动为你排序并替换成上标 [1]。**绝不可**自行输出普通的 [1] 或 (Author, Year) 格式。\n"

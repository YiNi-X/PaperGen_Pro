...
def call_deepseek_write_chapter(
    ...
    if images_data:
        ...
        for img in images_data:
            ...
            context_hint = img.get('caption_context', '无')
            image_descriptions += f"- 图片ID: {img.get('id', 'unknown')}\n  原始图注与上下文: {context_hint}\n"
    ...
    system_prompt = (
        ...
        "4. **文献引用规范**：在文中陈述他人观点或使用数据时，必须极其自然地插入引用标记。你**只能**使用「可用参考文献列表」中的文献。引用时，**必须**输出占位符标记：`[REF_{文献ID}]`，例如 `[REF_ref_001]`。系统随后会自动为你排序并替换成上标 `[1]`。**绝不可**自行输出普通的 `[1]` 或 `(Author, Year)` 格式。\n"
        "5. **高级 LaTeX 排版**：如果涉及数学表达式、物理量、变量字母（如 x, y, α），无论是独立公式还是行内变量，**必须**使用 LaTeX 格式输出（例如独立公式使用 `$$ E=mc^2 $$`，行内使用 `$ x_1 $`）。如果原文素材中有公式，在此处引用时务必原样或优化后呈现。\n"
        ...
        "8. **智能插图引擎**：如果你认为该章节当前讲述的内容可以通过「可用图片资源列表」中的某张图片进行更直观的展示印证，请**必须**在段落之间的独立一行输出占位符标记：`[INSERT_IMG_{图片ID}]`，例如 `[INSERT_IMG_img_001]`。\n"
